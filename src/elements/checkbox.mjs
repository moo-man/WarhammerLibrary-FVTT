...
export default class CheckboxElement extends AdoptedStyleSheetMixin(
  ...
  /** @override */
  static tagName = "warhammer-checkbox";
  ...
  static CSS = `
    ...
    :host > div {
      ...
      border: var(--checkbox-border-width, 2px) solid var(--checkbox-border-color, var(--color-gold));
    ...
    :host([checked]) :is(.checked, .disabled, .indeterminate) {
      background: var(--checkbox-fill-color, var(--color-gold));
    }
