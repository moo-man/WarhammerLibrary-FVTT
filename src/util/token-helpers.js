    ...
    static async moveAuras(token, update, options, user)
    {
        if (game.users.activeGM.id == game.user.id)
        {
            ...
            if (update.x || update.y || update.hidden)
            {
                ...
                {
                    CanvasAnimation.animate([{parent: t.object, attribute : "x", to: options._newCenter[token.id].x}, {parent: t.object, attribute : "y", to: options._newCenter[token.id].y}], {duration : tokenAnimations[0].duration});
                }
