...
export default class CompendiumBrowser extends WarhammerSheetMixinV2(HandlebarsApplicationMixin(ApplicationV2)) {
  ...
  _configureRenderOptions(options) {
    ...
    if ( options.isFirstRender ) {
      const tab = this.constructor.TABS.find(t => t.tab === this.options.tab);
      if ( tab ) foundry.utils.setProperty(options, "browser.types", tab.types);
    }
  }
  ...
  async _prepareSidebarContext(partId, context, options) {
    ...
    const types = foundry.utils.getProperty(options, "browser.types") ?? [];
  ...
  static async fetch(documentClass, { types=new Set(), filters=[], index=true, indexFields=new Set(), sort=true }={}) {
    ...
    let documents = game.packs
      ...
      .concat(
        // @todo separate World from Sources
